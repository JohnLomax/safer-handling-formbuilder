                    ...
                    <div class="brand-panel">
                        ...
                        <div class="mt-4 space-y-3">
                            ...
                            <div class="rounded-[12px] border border-sh-border bg-white/70 p-4">
                                ...
                                @endif
                                @if ($enquiry->preferredDateTimeLabel() !== '' && ! $enquiry->booking_submitted_at)
                                    <p class="mt-1 text-xs text-sh-mid">Preferred date: {{ $enquiry->preferredDateTimeLabel() }}</p>
                                @endif
                                @if (! empty($acceptVenueEmailBlockedReason) && ! $enquiry->booking_submitted_at)
                                    <p class="mt-2 text-xs text-amber-800">{{ $acceptVenueEmailBlockedReason }}</p>
                                @endif
                                ...
                                <div class="mt-3 flex flex-wrap gap-2">
                                    ...
                                    @if (in_array('booking_email', $retryableActions, true))
                                        <form method="POST" action="{{ route('admin.enquiries.retry.booking-email', $enquiry) }}">
                                            @csrf
                                            <button type="submit" class="btn-brand-outline text-xs inline-flex items-center gap-1.5" title="Send Accept Quote / venue details email">
                                                <svg class="h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                    <path d="M3 4a2 2 0 00-2 2v1.161l8.441 4.221a1.25 1.25 0 001.118 0L19 7.162V6a2 2 0 00-2-2H3z" />
                                                    <path d="M19 8.839l-7.77 3.885a2.75 2.75 0 01-2.46 0L1 8.839V14a2 2 0 002 2h14a2 2 0 002-2V8.839z" />
                                                </svg>
                                    ...
                                    @elseif (! empty($canResendBookingEmail))
                                        <form method="POST" action="{{ route('admin.enquiries.resend.booking-email', $enquiry) }}">
                                            @csrf
                                            <button type="submit" class="btn-brand-outline text-xs inline-flex items-center gap-1.5" title="Resend Accept Quote / venue details email">
                                                <svg class="h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                    <path d="M3 4a2 2 0 00-2 2v1.161l8.441 4.221a1.25 1.25 0 001.118 0L19 7.162V6a2 2 0 00-2-2H3z" />
                                                    <path d="M19 8.839l-7.77 3.885a2.75 2.75 0 01-2.46 0L1 8.839V14a2 2 0 002 2h14a2 2 0 002-2V8.839z" />
                                                </svg>
                                        ...
                                        <div class="flex shrink-0 flex-wrap items-center gap-2">
                                            ...
                                            @if ($isAcceptTermsEmailJourneyEvent)
                                                @if (in_array('booking_email', $retryableActions, true))
                                                    <form method="POST" action="{{ route('admin.enquiries.retry.booking-email', $enquiry) }}">
                                                        @csrf
                                                        <button type="submit" class="btn-brand-outline px-2.5 py-1 text-xs inline-flex items-center gap-1.5" title="Send Accept Quote / venue details email">
                                                            <svg class="h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                                <path d="M3 4a2 2 0 00-2 2v1.161l8.441 4.221a1.25 1.25 0 001.118 0L19 7.162V6a2 2 0 00-2-2H3z" />
                                                                <path d="M19 8.839l-7.77 3.885a2.75 2.75 0 01-2.46 0L1 8.839V14a2 2 0 002 2h14a2 2 0 002-2V8.839z" />
                                                            </svg>
                                                ...
                                                @elseif (! empty($canResendBookingEmail))
                                                    <form method="POST" action="{{ route('admin.enquiries.resend.booking-email', $enquiry) }}">
                                                        @csrf
                                                        <button type="submit" class="btn-brand-outline px-2.5 py-1 text-xs inline-flex items-center gap-1.5" title="Resend Accept Quote / venue details email">
                                                            <svg class="h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                                <path d="M3 4a2 2 0 00-2 2v1.161l8.441 4.221a1.25 1.25 0 001.118 0L19 7.162V6a2 2 0 00-2-2H3z" />
                                                                <path d="M19 8.839l-7.77 3.885a2.75 2.75 0 01-2.46 0L1 8.839V14a2 2 0 002 2h14a2 2 0 002-2V8.839z" />
                                                            </svg>
